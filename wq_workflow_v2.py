...
# ── New field combos for SC breakthrough (price/volume fields, not fundamentals) ──
# These fields are NOT heavily used in existing Active Alphas, reducing SC collision.
NEW_SUB_DELTA_EXPS = [
    "rank(close) - rank(ts_delta(volume,10))",
    "rank(vwap/close) - rank(ts_delta(returns,5))",
    "rank(close/open) - rank(ts_zscore(volume,20))",
    "rank(close) - 1.0*rank(ts_std(volume,10))",
    "rank(ts_delta(close,3)) - rank(ts_delta(volume,5))",
    "rank(log(volume)) - rank(ts_delta(close,5))",
    "rank(ts_mean(close,5)/close) - rank(ts_delta(volume,10))",
]
...
def _generate_sub_candidates(ratio_pool: list, ortho: dict, active_exprs: list) -> list:
    """Generate subtraction skeleton candidates.
    
    CRITICAL FIX: Original version mixed quarterly fundamentals with daily ts_*
    (e.g. rank(ebitda/cap) - rank(ts_delta(adv20,5))), causing WQ coverage
    mismatch → S=None for ALL candidates.
    
    New strategy: use fundamental growth rates via ts_delta on the RATIO itself.
    ts_delta(rank(A/B), N) preserves the fundamental frequency and avoids coverage
    mismatch. Pattern verified working: -1*rank(ts_delta(close,5)) + rank(oi/equity)
    (E5kZ7p60). We invert: rank(ts_delta(fund_ratio, N)) - rank(ts_delta(daily, M))
    where both sides are broadcast to daily but the fundamental side dominates.
    
    When ratio_pool is empty or all zero-usage, fall back to verified pairs only.
    """
    candidates = []
    seen = set()
    
    # Strategy 1: Fundamental growth rate (ts_delta on ratio)
    # This is the only approach that has proven to avoid S=None for fundamentals
    for (ratio_str, ratio_name) in ratio_pool[:10]:
        # Extract fields from ratio for naming
        parts = ratio_name.split("_")
        r_name = parts[0] if parts else ratio_name
        
        for period in [5, 10, 15]:
            expr = f"rank(ts_delta({ratio_str}, {period})) - rank(ts_delta(adv20, 5))"
            if expr not in seen and len(expr) < 200:
                seen.add(expr)
                score = score_candidate_orthogonality(expr, ortho, active_exprs)
                name = f"S_{r_name}_g{period}"[:40]
                name = name.replace("-","_").replace(" ","")
                candidates.append({
                    "name": name, "expr": expr,
                    "orthogonality_score": score,
    ...
    # Strategy 2: Pure fundamental ratio pair subtraction (no ts_*)
    # WARNING: rank(A/B) - rank(C/D) has NO ts_* operator → S=None always.
    # So we skip this strategy entirely. Only Strategy 1 and 3 are used.
    
    # Strategy 3: Cross-gate (sign of fundamental signal * daily signal)
    # Pattern from working alphas: rank(oi/equity) * rank(ts_mean(adv20,5))
    # But this is multiplication, not subtraction. For SUB skeleton, use:
    # sign(ts_delta(fund_ratio, N)) * rank(ts_delta(daily, M)) 
    # Actually this is cross_gate skeleton, not SUB. Skip for SUB category.
...
def _generate_price_volume_candidates(ortho: dict, active_exprs: list) -> list:
    """
    Generate subtraction candidates using ONLY price/volume fields.
    Strategy: These fields have lower SC collision because active alphas
    heavily use fundamentals (revenue, debt, equity, etc.) while price/volume
    combos are underutilized. Pattern: rank(PV_A) - rank(ts_delta/PV_B).
    These avoid fundamental field overlap → lower SC values.
    """
    candidates = []
    # Price/volume expressions that don't use any fundamental fields
    pv_expressions = [
        ("rank(close) - rank(ts_delta(volume,10))", "PV_close_vol10"),
        ("rank(vwap/close) - rank(ts_delta(returns,5))", "PV_vwap_ret5"),
        ("rank(close/open) - rank(ts_zscore(volume,20))", "PV_co_zsc_vol20"),
        ("rank(close) - 1.0*rank(ts_std(volume,10))", "PV_close_std_vol10"),
        ("rank(ts_delta(close,3)) - rank(ts_delta(volume,5))", "PV_dc3_dv5"),
        ("rank(log(volume)) - rank(ts_delta(close,5))", "PV_logv_dc5"),
        ("rank(ts_mean(close,5)/close) - rank(ts_delta(volume,10))", "PV_mc5_dc10"),
    ]
    for expr, name in pv_expressions:
        score = score_candidate_orthogonality(expr, ortho, active_exprs)
        candidates.append({
            "name": name, "expr": expr,
            "orthogonality_score": score,
            "skeleton": "price_volume",
            "weight": 1.0,
        })
    candidates.sort(key=lambda x: -x["orthogonality_score"])
    return candidates
...
def _generate_direct_rank_candidates(ortho: dict, active_exprs: list) -> list:
    ...
    KEY FIX (2026-06-04): 186 attempts, 0 IS pass. Root cause:
    - Pattern 4 (rank(X)+W*rank(returns)): "add" with returns = pure momentum = negative SHARPE
    - Pattern 3 (ts_rank(fund)): ts_rank on raw fund = noisy, low signal
    - Pattern 5/6/7 (ts_corr/zscore/ts_rank on pv1): low IS pass rate
    
    New strategy: mirror SUB's success pattern exactly:
    - Subtract, not add: rank(fund) - W*rank(ts_delta(pv1, N))
    - Use proven fields: debt, equity, operating_income (NOT cap which is size-biased)
    - Use proven operators: ts_delta(close/high, N) — proven SC-safe in SUB
    
    Additionally: add rank(fund) + W*rank(ts_decay_linear(pv1, N)) for
    mean-reversion momentum (decay_linear penalizes distant outliers).
    """
    candidates = []
    fund_fields = ["revenue", "debt", "equity", "operating_income"]
    pv1_fields = ["close", "high", "low"]

    # ── Pattern 1: ts_delta on pv1 (SUB-proven formula) ──
    # This mirrors SUB's 100% SC pass rate
    for fund in fund_fields:
        for pv1 in pv1_fields:
            for window in [5, 7]:
                expr = f"rank({fund})-{1.0}*rank(ts_delta({pv1},{window}))"
                name = f"DR_{fund[:4]}_d{pv1[:3]}{window}"[:40]
                if expr not in active_exprs:
                    candidates.append({
                        "name": name, "expr": expr,
                        "orthogonality_score": score_candidate_orthogonality(expr, ortho, active_exprs),
                        "skeleton": SKELETON_DIRECT_RANK,
                        "weight": 1.0,
                    })
                # Also try weight 0.7
                expr2 = f"rank({fund})-{0.7}*rank(ts_delta({pv1},{window}))"
                name2 = f"DR_{fund[:4]}_d{pv1[:3]}{window}_w7"[:40]
                if expr2 not in active_exprs:
                    candidates.append({
                        "name": name2, "expr": expr2,
                        "orthogonality_score": score_candidate_orthogonality(expr2, ortho, active_exprs),
                        "skeleton": SKELETON_DIRECT_RANK,
                        "weight": 0.7,
                    })

    # ── Pattern 2: ts_decay_linear on pv1 (NEW — mean-reversion momentum) ──
    # decay_linear gives more weight to recent data → captures short-term reversal
    for fund in fund_fields:
        for pv1 in pv1_fields:
            for window in [5, 10]:
                expr = f"rank({fund})-{1.0}*rank(ts_decay_linear({pv1},{window}))"
                name = f"DR_{fund[:4]}_dl{pv1[:3]}{window}"[:40]
                if expr not in active_exprs:
                    candidates.append({
                        "name": name, "expr": expr,
                        "orthogonality_score": score_candidate_orthogonality(expr, ortho, active_exprs),
    ...
    # ── Pattern 3: ts_delta(fund, N) +/- rank(pv1) — FUNDamental delta ──
    # Instead of raw fund rank, use CHANGE in fundamentals
    # rank(ts_delta(fund, N)) - W*rank(pv1): companies with improving fundamentals
    for fund in fund_fields:
        for pv1 in ["close", "returns"]:
            for window in [5, 10]:
                expr = f"rank(ts_delta({fund},{window}))-{1.0}*rank({pv1})"
                name = f"DR_td{fund[:4]}_{pv1}{window}"[:40]
                if expr not in active_exprs:
                    candidates.append({
                        "name": name, "expr": expr,
                        "orthogonality_score": score_candidate_orthogonality(expr, ortho, active_exprs),
    ...
    # ── Pattern 4: ts_rank(ts_delta(fund, N), W) — rank the change ──
    # Double-time-series: ts_delta + ts_rank on same fundamental
    for fund in fund_fields:
        for window_d in [5, 10]:
            for window_r in [20, 60]:
                expr = f"ts_rank(ts_delta({fund},{window_d}),{window_r})"
                name = f"DR_trtd{fund[:4]}_d{window_d}_r{window_r}"[:40]
                if expr not in active_exprs:
                    candidates.append({
                        "name": name, "expr": expr,
                        "orthogonality_score": score_candidate_orthogonality(expr, ortho, active_exprs),
...
def _generate_ts_std_candidates(ortho: dict, active_exprs: list) -> list:
    """
    Gap-fill: ts_std — volatility-based signals.
    rank(ts_std(returns, N)) +/− rank(fund_ratio)
    ts_std is completely unused in any active alpha → highest novelty bonus.
    Captures: mean-reversion on volatility (high vol → low returns).
    """
    candidates = []
    fund_ratios = [
        ("debt/equity", "de"),
    ...
    # ts_std windows: short (5), medium (10), long (20)
    std_windows = [5, 10, 20]
    seen = set()
    for w in [0.5, 0.7, 1.0]:
        for window in std_windows:
            for (ratio_str, ratio_name) in fund_ratios:
                for op_sign in ["add", "subtract"]:
                    if op_sign == "add":
                        expr = f"rank(ts_std(returns,{window}))+{w}*rank({ratio_str})"
                    else:
                        expr = f"rank(ts_std(returns,{window}))-{w}*rank({ratio_str})"
                    ...
                    seen.add(expr)
                    score = score_candidate_orthogonality(expr, ortho, active_exprs)
                    name = f"TS_std_{ratio_name}_w{int(w*10)}_d{window}"[:40]
                    candidates.append({
                        "name": name, "expr": expr,
                        "orthogonality_score": score,
                        "skeleton": SKELETON_TSRANK_CORR,  # classify as TS_RANK_CORR group for rotation
                        "weight": w,
                    })
    candidates.sort(key=lambda x: -x["orthogonality_score"])
    return candidates
...
def _generate_volatility_decay_candidates(ortho: dict, active_exprs: list) -> list:
    """
    Gap-fill: volatility_decay — decay-linear momentum on price/vwap ratios.
    rank(ts_decay_linear(close/vwap, N)) +/− rank(fund)
    decay_linear is only used in 1 active (negatively).
    Captures: momentum mean-reversion using decay-weighted price vs VWAP.
    """
    candidates = []
    fund_fields = ["debt", "revenue", "operating_income", "cap", "equity"]
    # decay windows: 5, 10, 20
    decay_windows = [5, 10, 20]
    seen = set()
    for w in [0.5, 0.7, 1.0]:
        for window in decay_windows:
            for fund in fund_fields:
                for op_sign in ["add", "subtract"]:
                    if op_sign == "add":
                        expr = f"rank(ts_decay_linear(close/vwap,{window}))+{w}*rank({fund})"
                    else:
                        expr = f"rank(ts_decay_linear(close/vwap,{window}))-{w}*rank({fund})"
                    if expr in seen or expr in active_exprs:
                        continue
                    seen.add(expr)
                    score = score_candidate_orthogonality(expr, ortho, active_exprs)
                    name = f"VL_decay_{fund}_w{int(w*10)}_d{window}"[:40]
                    candidates.append({
                        "name": name, "expr": expr,
                        "orthogonality_score": score,
                        "skeleton": SKELETON_TREND_BREAK,
                        "weight": w,
                    })
    candidates.sort(key=lambda x: -x["orthogonality_score"])
    return candidates
...
def _generate_zscore_momentum_candidates(ortho: dict, active_exprs: list) -> list:
    """
    Gap-fill: zscore_momentum — positive zscore signals.
    rank(ts_zscore(pv1, N)) + rank(fund_ratio)
    Current active has ts_zscore but only with negative sign (−rank(ts_zscore)).
    Captures: zscore deviation → fundamental quality (zscore high → good fundamentals).
    """
    candidates = []
    fund_ratios = [
        ("debt/equity", "de"),
    ...
    zscore_fields = ["close", "volume", "high"]
    zscore_windows = [10, 20, 30]
    seen = set()
    for w in [0.3, 0.5, 0.7, 1.0]:
        for field in zscore_fields:
            for window in zscore_windows:
                for (ratio_str, ratio_name) in fund_ratios:
                    expr = f"rank(ts_zscore({field},{window}))+{w}*rank({ratio_str})"
                    if expr in seen or expr in active_exprs:
                        continue
                    seen.add(expr)
                    score = score_candidate_orthogonality(expr, ortho, active_exprs)
                    name = f"ZS_{field}{window}_{ratio_name}_w{int(w*10)}"[:40]
                    candidates.append({
                        "name": name, "expr": expr,
                        "orthogonality_score": score,
...
def _generate_three_term_candidates(ortho: dict, active_exprs: list) -> list:
    ...
    KEY FIX (2026-06-04): 185 attempts, 7 IS pass (3.8%), 0 SC pass.
    Root cause: TWO FUND RATIOS create high self-correlation with MULT alphas
    (all active alphas use rev/ev, oi/equity, de ratios). 
    
    Strategy:
    1. Use CROSS-GENRE ratio pairs (fund ratio + pv1 rank) instead of two fund ratios
       This creates structural distance from active MULT alphas → lower SC
    2. Use ts_delta (not ts_mean) on momentum side — proven SC-safe
    3. Keep weights low (0.3-0.5) to minimize self-correlation from the third term
    """
    candidates = []
    
    # ── Pattern 1: 2 fund ratios - ts_delta(pv1) (original proven IS pattern) ──
    # Keep a SMALL set of these with low weights
    fund_ratios = [
        ("revenue/enterprise_value", "rev_ev"),
        ("operating_income/cap", "oi_cap"),
        ("debt/equity", "de"),
    ...
    for (r1_str, r1_name) in fund_ratios:
        for (r2_str, r2_name) in fund_ratios:
            ...
            for (sig_str, sig_name) in pv1_signals:
                for w in [0.3, 0.5]:  # LOWER weights → less self-correlation
    ...
    # ── Pattern 2: cross-genre (fund ratio + single fund rank) - ts_delta(pv1) ──
    # Instead of two ratios, use one ratio + one raw fund field
    # This breaks the pattern-matching against active MULT alphas
    fund_single = ["debt", "equity", "operating_income", "revenue"]
    for (ratio_str, ratio_name) in fund_ratios:
        for fund in fund_single:
            if ratio_str.endswith(f"/{fund}") or ratio_str.startswith(f"{fund}/"):
                continue  # avoid overlap
            for (sig_str, sig_name) in pv1_signals:
                for w in [0.3, 0.5]:
                    expr = f"rank({ratio_str})+rank({fund})-{w}*rank({sig_str})"
                    if expr in seen or expr in active_exprs:
                        continue
                    seen.add(expr)
                    score = score_candidate_orthogonality(expr, ortho, active_exprs)
                    name = f"3T_{ratio_name}_{fund[:3]}_{sig_name}_w{int(w*10)}"[:40]
                    name = name.replace("-","_").replace(" ","")
                    candidates.append({
                        "name": name, "expr": expr,
                        "orthogonality_score": score,
    ...
    # ── Pattern 3: 3 fund fields (no ratios) - ts_mean(pv1) ──
    # rank(fund1) + rank(fund2) + rank(fund3) - rank(ts_mean(pv1,N))
    # All different fields → minimal self-correlation
    for f1 in ["debt", "equity", "operating_income"]:
        for f2 in ["revenue", "cap"]:
            if f1 == f2:
                continue
            for f3 in ["enterprise_value"]:
                for w in [0.3, 0.5]:
                    expr = f"rank({f1})+rank({f2})-{w}*rank(ts_mean(close,5))"
                    if expr in seen or expr in active_exprs:
                        continue
                    seen.add(expr)
                    score = score_candidate_orthogonality(expr, ortho, active_exprs)
                    name = f"3T_{f1[:3]}{f2[:3]}_{f3[:3]}_mcl5_w{int(w*10)}"[:40]
                    name = name.replace("-","_").replace(" ","")
                    candidates.append({
                        "name": name, "expr": expr,
                        "orthogonality_score": score,
...
def generate_candidates(ortho: dict, active_exprs: list, n: int = 3,
                        failed_exprs: list = None, stuck_batches: int = 0,
                        skeleton_popularity: dict = None) -> list:
    """
    ...
    NEW: Skip skeleton types with SC fail_rate >= 90% to avoid waste.
    """
    ratio_pool = _build_ratio_pool(ortho, skip_zero_occupancy=(stuck_batches >= 1))
    
    # ── Skeleton filter: skip skeleton types that have SC fail_rate >= 90% ──
    # This prevents wasting time on skeleton types that consistently fail SC checks
    blocked_skeletons = set()
    if skeleton_popularity:
        for sk, data in skeleton_popularity.items():
            attempts = data.get("attempts", 0)
            sc_pass = data.get("sc_pass", 0)
            sc_fail = data.get("sc_fail", 0)
            sc_total = sc_pass + sc_fail
            if sc_total >= 2:  # At least 2 SC attempts
                fail_rate = sc_fail / max(sc_total, 1)
                if fail_rate >= 0.9:
                    blocked_skeletons.add(sk)
                    log(f"  ⛔ Skeleton '{sk}' blocked (SC fail rate: {fail_rate:.0%}, attempts={attempts})")
    
    mult_count = ortho.get("multiplication_count", 0)
    sub_count = ortho.get("subtraction_count", 0)
    
    log(f"  🏗 AST structure balance: {mult_count} mult / {sub_count} sub in active")
    if blocked_skeletons:
        log(f"  🚫 Blocked skeleton types: {blocked_skeletons}")
    ...
    if ratio_pool:
        ...
        if use_mult and SKELETON_MULT not in blocked_skeletons:
        ...
        elif use_mult and SKELETON_MULT in blocked_skeletons:
            log("  ⚠️ MULTIPLICATION skeleton blocked (SC fail rate too high), falling back to SUB")
            use_sub = True
    ...
    if growth_mul and SKELETON_MULT not in blocked_skeletons:
        log(f"  ✖️ Adding {len(growth_mul)} multiplication candidates (preferred over sub)")
        all_candidates.extend(growth_mul)
    elif growth_mul and SKELETON_MULT in blocked_skeletons:
        log(f"  ⛔ Growth multiplication blocked (SC fail rate too high), skipping")
        growth_mul = []

    # ── PRICE/VOLUME GENERATOR: SC breakthrough attempt ──
    # Pure price/volume expressions avoid fundamental field overlap → lower SC values
    pv_cands = _generate_price_volume_candidates(ortho, active_exprs)
    if pv_cands:
        log(f"  📊 Adding {len(pv_cands)} price/volume candidates (SC breakthrough)")
        for pc in pv_cands:
            log(f"    PV: {pc['expr'][:70]}")
    ...
    if ts_std:
        ...
        all_candidates.extend(ts_std)

    vol_decay = _generate_volatility_decay_candidates(ortho, active_exprs)
    if vol_decay:
        log(f"  🌊 Adding {len(vol_decay)} VOL_DECAY candidates (decay-linear momentum)")
        all_candidates.extend(vol_decay)

    zscore_mom = _generate_zscore_momentum_candidates(ortho, active_exprs)
    if zscore_mom:
        log(f"  📊 Adding {len(zscore_mom)} ZSCORE_MOM candidates (positive zscore)")
        all_candidates.extend(zscore_mom)
    ...
    for c in deduped:
        ...
        if fail_count > 5:
            ...
            base_score -= 0.5
        # Apply P1b: SC-aware skeleton bonus/penalty from skeleton_popularity
        if sk in skeleton_popularity:
            sk_data = skeleton_popularity[sk]
            sc_pass_rate = sk_data.get("sc_pass_rate", 0.0)
            if sc_pass_rate == 0.0 and sk_data.get("attempts", 0) >= 2:
                base_score -= 3.0
            elif sc_pass_rate > 0.0:
                base_score += sc_pass_rate * 2.0
        ...
        base_score += topo
        # ── P8: Over-generation penalty ──
        if sk in skeleton_popularity:
            sk_data = skeleton_popularity[sk]
            total_attempts = sk_data.get("attempts", 0)
        ...
        # ── Extra penalty for blocked skeletons (SC fail rate >= 90%) ──
        # This ensures blocked skeletons never get selected
        if sk in blocked_skeletons:
            c["composite_score"] = -999.0  # Effectively infinite penalty
            c["blocked"] = True
...
class Workflow:
    ...
    def _main_loop(self):
        while self.running:
            ...
            # 2.6. Pre-batch: Process IS_PASS alphas pending SC (critical fix)
            # These alphas were IS_PASS but never reached SC due to pipeline bugs
            ...
            candidates = generate_candidates(
                ...
                skeleton_popularity=self.state.get("skeleton_popularity", {}),
            )
            if not candidates:
            ...
            for idx, cand in enumerate(candidates):
                ...
                if not success:
                    ...
                    if not success:
                        ...
                        # ── DELETE SC-failed alpha from WQ to free resources ──
                        alpha_id = cand.get("alpha_id")
                        if alpha_id:
                            try:
                                log(f"  🗑️  Deleting SC-failed alpha {alpha_id} ({cand.get('name')})")
                                self.s.delete(f"{API}/alphas/{alpha_id}", timeout=15)
                            except Exception as e:
                                log(f"  ⚠️  Delete failed: {e}")
                        continue
    ...
    def _process_pending_is_pass(self) -> int:
        """Process IS_PASS alphas from alpha_stats that haven't been SC-tested.
        
        These are alphas that passed IS but were never submitted to SC due to:
        1. Pipeline bugs before v3.10 (tune-first-lost-alpha)
        2. S=None dead loop consuming all batch capacity after v3.10
        
        Returns: number of alphas processed (SC attempted)
        """
        try:
            # Query for IS_PASS alphas without SC result
            query = """
                SELECT alpha_id, name, expr, sharpe, fitness 
                FROM alpha_stats 
                WHERE is_status='PASS' AND sc_value IS NULL
                ORDER BY sharpe DESC
            """
            rows = _wqdb.execute_query(query)
            if not rows:
                log("  ℹ️ No IS_PASS alphas pending SC")
                return 0
            
            count = 0
            for row in rows:
                alpha_id, name, expr, sharpe, fitness = row
                if not self.running:
                    break
                
                log(f"  🔧 Processing pending IS_PASS: {name} (S={sharpe}, F={fitness})")
                ...
                # Track skeleton outcome
                self._track_skeleton_outcome(cand, "is_pass", sharpe)
                
                # Submit to SC
                self.state["phase"] = "sc_submit"
                self.save_checkpoint()
                
                log(f"    ⏳ Submitting to SC queue for {name}...")
                success = self._run_sc(cand)
                count += 1
                ...
                if not success:
                    ...
                    if not success:
                        ...
                        self._track_skeleton_outcome(cand, "sc_fail", sharpe, cand.get("sc_value"))
                else:
                    # SC passed — track and submit to ACTIVE
                    self._track_skeleton_outcome(cand, "sc_pass", sharpe, cand.get("sc_value"))
                    self.state["phase"] = "submit"
                    self.save_checkpoint()
                    self._submit_alpha(cand, alpha_id)
            
            if count > 0:
                log(f"  ✅ Processed {count} pending IS_PASS alpha(s) for SC")
            
            return count
            
        except Exception as e:
            log(f"  ⚠️ Error processing pending IS_PASS alphas: {e}", "error")
            return 0
    ...
    def _run_sc(self, cand: dict) -> bool:
        ...
        # POST /alphas/{id}/submit with body → WQ returns SC check result immediately in 403
        # This is KEY: the 403 response contains the SC value!
        try:
            r = self.s.post(f"{API}/alphas/{alpha_id}/submit", json={
                "universe": "TOP3000",
                "region": "GLOBAL",
                "delay": 0,
                "decay": 0,
                "neutralization": "INDUSTRIAL",
                "truncation": 0.1,
                "pasteurization": True,
                "unitHandling": "VERIFY",
                "nanHandling": "OFF",
                "language": "PINVOKE",
                "visualization": "STAIRS"
            }, timeout=30)
        except Exception as e:
        ...
        if r.status_code == 403:
            ...
            if sc_result is not None and sc_result.get("result") == "PASS":
                ...
                log(f"⏸️ SC for {cand['name']} aborted/stuck", "warn")
    ...
    def _track_skeleton_outcome(self, cand: dict, outcome: str,
        ...
        if outcome in ("sc_pass",):
            sk_data["sc_pass"] += 1
        if outcome in ("sc_fail",):
            sk_data["sc_fail"] = sk_data.get("sc_fail", 0) + 1
        # sc_pass_rate should be sc_pass / (sc_pass + sc_fail), not sc_pass / attempts
        sk_sc_total = sk_data["sc_pass"] + sk_data.get("sc_fail", 0)
        sk_data["sc_pass_rate"] = sk_data["sc_pass"] / max(sk_sc_total, 1)
    ...
    def _tune_and_retry(self, cand: dict, ortho: dict, failed_phase: str,
        ...
        if failed_phase == "is":
            ...
            # ── SC fail: cross-skeleton escape (v5) ──
            # Root cause: SC failure is NOT about fields — it's about structural self-correlation.
            # The same skeleton (e.g. mult_ratio) repeatedly collides with existing ACTIVE alphas.
            # Strategy: (1) Check if this skeleton has 2+ SC failures → if so, SKIP to new skeleton
            #           (2) Try neutralization on same expr (quick wins)
            #           (3) AST topology mutations (different skeletons)
            #           (4) Generate cross-skeleton candidates from the generator pool
            
            base_expr = cand.get("expr", "")
            base_name = cand.get("name", "orig")
            base_skeleton = cand.get("skeleton", "unknown")
            state = self.state
            
            # Cross-skeleton exhaustion check:
            # Count SC failures per skeleton from skeleton_popularity
            pop = state.get("skeleton_popularity", {})
            sk_sc_fail_rate = 0.0
            if base_skeleton in pop:
                sk_data = pop[base_skeleton]
                sk_sc_fail_rate = 1.0 - sk_data.get("sc_pass_rate", 0.0)
            else:
                sk_sc_fail_rate = 0.0
            
            log(f"  🔬 SC cross-skeleton escape on '{base_name}' (skel={base_skeleton})")
            log(f"    Skeleton sc_fail_rate: {sk_sc_fail_rate:.2f}")
            
            # Phase 0: If skeleton has high SC fail rate (sc_pass_rate < 0.3) AND
            # the skeleton has more than 1 failed attempt, skip to new skeleton
            skip_to_new_skeleton = (
                base_skeleton in pop and
                pop[base_skeleton].get("sc_pass_rate", 0) < 0.3 and
                pop[base_skeleton].get("attempts", 0) >= 2
            )
            
            if skip_to_new_skeleton:
                log(f"  🔄 Skeleton {base_skeleton} exhausted (sc_pass_rate={pop[base_skeleton].get('sc_pass_rate', 0):.2f}, attempts={pop[base_skeleton].get('attempts', 0)}), skipping to new skeleton")
            else:
                # Phase 1: Neutralization dimension degradation (same expr — quick win)
                # Only try if we haven't already exhausted this skeleton
            ...
            # Phase 3: Cross-skeleton fallback — generate candidates from DIFFERENT skeleton types
            # When current skeleton is exhausted OR all AST mutations failed,
            # generate candidates from the generator pool with a different skeleton focus
            log(f"  🔄 Phase 3: Generating cross-skeleton candidates...")
            cross_skeletons = []
            for sk_candidate in [
                ("sub_delta", _generate_sub_candidates),
                ("direct_rank", _generate_direct_rank_candidates),
                ("three_term", _generate_three_term_candidates),
            ]:
                sk_name, gen_func = sk_candidate
                active_exprs = self._get_active_expressions()
                cross_skeletons.append((sk_name, gen_func(ortho, active_exprs)))
            
            # Sort by orthogonality score and take top candidates from each cross-skeleton
            cross_cands = []
            for sk_name, candidates in cross_skeletons:
                for c in candidates[:3]:  # Top 3 from each cross-skeleton
                    c["cross_skeleton"] = sk_name
                    cross_cands.append(c)
            
            cross_cands.sort(key=lambda x: -x.get("orthogonality_score", 0))
            
            if cross_cands:
                log(f"  Generated {len(cross_cands)} cross-skeleton candidates")
                for cc in cross_cands[:5]:
                    log(f"    [{cc['cross_skeleton']}] {cc['name']}: {cc['expr'][:80]}")
                
                # Try cross-skeleton candidates
                for cc in cross_cands:
                    if not self.running:
                        return False
                    var_name = f"cross_{cc['cross_skeleton']}_{cc['name']}"[:40]
                    log(f"    Cross-skeleton attempt: {cc['cross_skeleton']} → {cc['name']}")
                    log(f"      expr: {cc['expr']}")
                    
                    override = dict(DEFAULT_SETTINGS) if settings_override else dict(DEFAULT_SETTINGS)
                    payload = {"type": "REGULAR", "regular": cc["expr"], "settings": override}
                    success = self._run_single_sim_and_sc(var_name, payload, cand, failed_phase, max_tune_attempts, is_optimization)
                    if success:
                        log(f"  ✅ SC passed via cross-skeleton: {cc['cross_skeleton']}")
                        return True
                    else:
            ...
            if not skip_to_new_skeleton:
                ...
                denoms = [d for d in fund_denoms if field_usage.get(d, 0) <= 2]
                ...
                for num in nums[:6]:
                    for den in denoms[:4]:
                        ...
                        if pair_count >= max_tune_attempts:
                            break
                if pair_count >= max_tune_attempts:
                    pass  # All pairs tried
        else:
            # skip_to_new_skeleton is True — no Phase 4, just report
            pass
